...
const TicketsCreatedByTable = () => {
    ...
    const fetchTickets = (id) => {
        fetch(`${serverUrl}/ticket/assigned-by?id=${id}`, {
            credentials: 'include',
        })
        .then(res => res.json())
        .then(data => {
    ...
    return (
        <section className='w-screen h-screen p-4'>
            <Header activeHref='/tickets-by' />
            
            <div class="border-base-content/25 w-full h-4/5 overflow-x-auto border">
                <div className='px-4 pt-4 pb-2 flex justify-between'>
                    <h1 className='text-2xl'>{user.username ? `Tickets Created by ${user.username}` : 'Tickets You Created'}</h1>
...
export default TicketsCreatedByTable;